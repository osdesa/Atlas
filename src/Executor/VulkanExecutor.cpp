...
namespace Atlas
{
    ...
    struct VulkanExecutor::Impl final
    {
        ...
        /// @brief Releases executor-owned Vulkan objects in reverse dependency order.
        void destroyReusableResources() noexcept
        {
        ...
        bool submit(TaskHandle taskHandle, VulkanDispatch&& dispatch, CompletionChannel& completionChannel)
        {
...

            {
                ...
                pending.emplace_back(WorkItem{ dispatch,
                                               TaskCompletion{ taskHandle, nullptr, std::chrono::nanoseconds{ 0 },
        ...
        std::optional<std::chrono::nanoseconds> execute(const VulkanDispatch& dispatch)
        {
            ...
            vkUpdateDescriptorSets(context->device, static_cast<std::uint32_t>(descriptorWrites.size()), descriptorWrites.data(), 0U,
                                   nullptr);
            ...
            vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipeline->pipelineLayout, 0U, 1U, &descriptorSet,
                                    0U, nullptr);
            ...
            context->checkDeviceResult(
                vkWaitForFences(context->device, 1U, &fence, VK_TRUE, std::numeric_limits<std::uint64_t>::max()), "vkWaitForFences");
            if (context->executorFaultInjector)
            {
            ...
            if (timestampQueryPool != VK_NULL_HANDLE)
            {
                ...
                context->checkDeviceResult(
                    vkGetQueryPoolResults(context->device, timestampQueryPool, 0U, 2U, sizeof(queryResults), queryResults.data(),
